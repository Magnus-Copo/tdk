...
const VisionMissionSection = () => {
    ...
    return (
        <section id="vision-mission" style={{ backgroundColor: currentTheme.bg, padding: 'clamp(60px, 10vw, 140px) clamp(16px, 4vw, 24px)', transition: 'background-color 0.8s ease', minHeight: '400px', position: 'relative', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.5), inset 0 -1px 0 rgba(0,0,0,0.05)' }}>
            <div style={{ maxWidth: '1000px', margin: '0 auto', position: 'relative', zIndex: 1 }}>

                {/* 1. MILLER'S LAW: TABBED INSTITUTIONS */}
                <div style={{ background: '#00000008', padding: '8px', borderRadius: '24px', display: 'flex', marginBottom: 'clamp(32px, 6vw, 80px)', border: '1px solid #00000008', flexDirection: 'row', flexWrap: 'wrap', gap: '4px' }}>
                    <TabButton
                        id="sktec"
                        label="SKTEC Trust"
                ...
                <div key={activeTab} style={{ opacity: 1, transition: 'opacity 0.3s ease' }}>
                        {activeTab === 'sktec' ? (
                            <div>
...
const AccordionItem = ({ items, isOpen, onToggle, theme }) => (
    <div style={{ marginBottom: '16px' }}>
        ...
        {isOpen && (
            ...
            >
                <div style={{ padding: 'clamp(16px, 3vw, 32px)', background: '#FFFFFF80', borderRadius: '0 0 24px 24px', border: `1px solid ${theme.primary}05`, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2vw, 24px)' }}>
                        {items.map((item, i) => (
